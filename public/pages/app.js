(function (root, factory) {
  ...
  const ROUTES = Object.freeze({
    home: 'EscapeSync Phase 1.html',
    builder: 'EscapeSync Phase 2.html',
    command: 'EscapeSync Phase 3.html',
    map: 'EscapeSync Phase 4.html',
    rescue: 'EscapeSync Phase 5.html',
    budget: 'EscapeSync Phase 6.html',
    pip: 'EscapeSync Phase 7.html',
    mobile: 'EscapeSync Phase 8.html',
    demo: 'EscapeSync Phase 9.html',
    history: 'EscapeSync History.html',
    settings: 'EscapeSync Settings.html'
  });
  ...
  const cleanBool = (value, fallback) => typeof value === 'boolean' ? value : fallback;
  ...
  function normalizeState(input) {
    ...
    return {
      ...
      currency: cleanText(value.currency, DEFAULT_STATE.currency),
      language: cleanText(value.language, DEFAULT_STATE.language),
      units: cleanText(value.units, DEFAULT_STATE.units),
      weatherAlerts: cleanBool(value.weatherAlerts, DEFAULT_STATE.weatherAlerts),
      budgetAlerts: cleanBool(value.budgetAlerts, DEFAULT_STATE.budgetAlerts),
      delayAlerts: cleanBool(value.delayAlerts, DEFAULT_STATE.delayAlerts),
      crewVotes: cleanBool(value.crewVotes, DEFAULT_STATE.crewVotes),
      reduceMotion: cleanBool(value.reduceMotion, DEFAULT_STATE.reduceMotion),
      shareLocation: cleanBool(value.shareLocation, DEFAULT_STATE.shareLocation)
    };
  }
  ...
  function runAction(name, trigger) {
    ...
    switch (name) {
      ...
      case 'pricing': case 'about': case 'sign-in': case 'preview-change': {
  ...
  const wiki = (path, file) => ({
    src: WIKI + path,
    credit: 'Wikimedia Commons',
    creditHref: 'https://commons.wikimedia.org/wiki/File:' + encodeURIComponent(file)
  });

  const PHOTOS = Object.freeze({
    alice: unsplash('1494790108377-be9c29b29330', 'Michael Dam', 'michaeldam'),
    ben: unsplash('1539571696357-5a69c17a67c6', 'ian dooley', 'sadswim'),
    chris: unsplash('1507003211169-0a1dd7228f2d', 'Joseph Gonzalez', 'miracletwentyone'),
    david: unsplash('1488161628813-04466f872be2', 'rayul', 'rayul'),
    alex: unsplash('1500648767791-00dcc994a43e', 'Christopher Campbell', 'chrisjoelcampbell'),
    dutchSquare: wiki('f/f0/Dutch_Square_Malacca.JPG', 'Dutch Square Malacca.JPG'),
    jonker: wiki('c/c0/Jonker_Street_Night_Market_in_Melaka%2C_Malaysia.jpg', 'Jonker Street Night Market in Melaka, Malaysia.jpg'),
    hangJebat: wiki('b/bc/Jalan_Hang_Jebat%2C_Melaka_%28211112%29_01.jpg', 'Jalan Hang Jebat, Melaka (211112) 01.jpg'),
    aFamosa: wiki('f/fb/A_Famosa_Fortress_Melaka.jpg', 'A Famosa Fortress Melaka.jpg'),
    river: wiki('6/6c/Melaka_River_Cruise.jpg', 'Melaka River Cruise.jpg'),
    cafe: wiki('6/6e/Melaka_Malaysia_Geographer-Cafe-at-Jonker-Walk-01.jpg', 'Melaka Malaysia Geographer-Cafe-at-Jonker-Walk-01.jpg'),
    laksa: wiki('1/1c/Nyonya_Laksa.jpg', 'Nyonya Laksa.jpg'),
    museum: wiki('3/36/Baba_Nyonya_Heritage_Museum.JPG', 'Baba Nyonya Heritage Museum.JPG'),
    museumInterior: wiki('7/71/Baba_Nyonya_Heritage_Museum_-_Reception_Hall.jpg', 'Baba Nyonya Heritage Museum - Reception Hall.jpg'),
    klebang: wiki('0/05/Klebang_Beach.JPG', 'Klebang Beach.JPG'),
    hotel: wiki('3/33/Hotel_lobby.jpg', 'Hotel lobby.jpg'),
    klSentral: wiki('f/f6/KL_Sentral.jpg', 'KL Sentral.jpg'),
    melakaSentral: wiki('e/e0/Melaka_Sentral.jpg', 'Melaka Sentral.jpg'),
    stPaul: wiki('b/bf/St._Paul%27s_Church_Malacca_2012.JPG', "St. Paul's Church Malacca 2012.JPG")
  });

  const PLACE_KEYS = Object.freeze({
    'KL Sentral Departure': 'klSentral',
    'Hotel Check-In': 'hotel',
    'Jonker Street': 'jonker',
    'A Famosa': 'aFamosa',
    Dinner: 'river',
    'Nyonya Breakfast': 'laksa',
    'Baba & Nyonya Museum': 'museumInterior',
    'Melaka River Cruise': 'river',
    'Sunset at Klebang': 'klebang',
    'Slow Café Morning': 'cafe',
    'Souvenir Run': 'hangJebat',
    'Coach Back to KL': 'melakaSentral',
    'Dutch Square': 'dutchSquare',
    "St. Paul's Hill": 'stPaul',
    'Nyonya Makko': 'laksa',
    'Capitol Satay Celup': 'laksa',
    'Riverine Kitchen': 'river',
    'The Pines Melaka': 'hotel',
    'Straits Heritage House': 'museum',
    'Klebang Bay Rooms': 'klebang',
    'Hainan Tea House': 'cafe',
    'Jonker Kopitiam': 'cafe',
    makko: 'laksa',
    capitol: 'laksa',
    riverine: 'river',
    pines: 'hotel',
  ...
  function photoFor(key) {
    ...
    const crew = PHOTOS[raw.toLowerCase()];
    if (crew && crew.src && ['alice', 'ben', 'chris', 'david', 'alex'].includes(raw.toLowerCase())) return crew;
    const mapped = PLACE_KEYS[raw] || PLACE_KEYS[raw.toLowerCase()];
    return PHOTOS[mapped] || PHOTOS.dutchSquare;
  }
